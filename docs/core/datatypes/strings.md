...
1. Dado un color en formato hexadecimal (ej. `#FF8800`), muéstralo formateado como `R: 255, G: 136, B: 0`.
2. Dado un nombre completo en formato `"Nombre Apellido"`, muéstralo invertido como `"Apellido, Nombre"`.
3. Dada una cadena con palabras separadas por comas, sepáralas y muéstralas numeradas, una por línea.
4. Dado el número de un NIF español (8 dígitos), calcula y muestra su letra de control usando la tabla de letras `TRWAGMYFPDXBNJZSQVHLCKE`.
5. Dada una cadena y un número `n`, muestra la cadena repetida `n` veces sin espacios entre las repeticiones.
6. Dadas dos cadenas, calcula cuántos caracteres tienen en común (sin repetir) y muestra el resultado.
7. Dada una línea de texto con un encabezado Markdown (ej. `## Título`), conviértela a su equivalente en HTML (ej. `<h2>Título</h2>`).
8. Dada una cadena de texto, cuenta cuántas veces aparece la palabra `"sheep"` (o su equivalente en español).
9. Dada una cadena con espacios al inicio y al final, elimina solo el primer y el último espacio (sin usar `strip()`).
10. Dada una expresión matemática como cadena (ej. `"integral(x^2)"`), extrae y muestra únicamente la parte entera del argumento.
11. Dado un nombre y un número, genera la frase `"Hello, {nombre}!"` repetida tantas veces como indique el número.
12. Dado un número entero como cadena, muestra su primer y último dígito.
