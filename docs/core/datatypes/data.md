...
Los programas están formados por **código** y **datos**[^1]. Pero a nivel interno de la memoria del computador (RAM) no son más que una secuencia de bits `100101101...` La interpretación de estos bits depende del lenguaje de programación, que almacena en la memoria no sólo el puro dato sino distintos metadatos[^2].
...
Por <span class="example">ejemplo:material-flash:</span> para nombrar una variable que almacene el _número de canciones_ en nuestro computador, podríamos usar `num_songs`.
