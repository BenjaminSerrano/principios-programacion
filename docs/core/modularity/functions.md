...
1. Escribe una función `calcular_imc(peso, altura)` que retorne el IMC (\(	ext{IMC} = rac{	ext{peso}}{	ext{altura}^2}\)). Úsala en un programa que solicite los datos al usuario y muestre el resultado.
2. Escribe una función `es_bisiesto(anio)` que retorne `True` si el año es bisiesto y `False` si no. Un año es bisiesto si es múltiplo de 4, excepto los múltiplos de 100 que no sean también múltiplos de 400.
3. Escribe una función `convertir_temperatura(valor, escala)` que convierta entre Celsius y Fahrenheit según la escala indicada (`"C"` o `"F"`).
4. Escribe una función `calcular_descuento(monto)` que retorne el porcentaje de descuento según el monto de compra. Úsala en un programa que calcule el precio final.
5. Escribe una función `validar_entero_positivo(mensaje)` que solicite un número entero positivo al usuario, lo valide y lo retorne. Úsala en los programas anteriores.
6. Escribe un programa con funciones que calcule el sueldo bruto y líquido de cada empleado de una empresa. El valor hora es \$2.500 y el líquido corresponde al 80% del bruto.
7. Escribe un programa con funciones que calcule el promedio de notas de un curso. Por cada estudiante se ingresan sus notas; al terminar se pregunta si se desea ingresar otro. Al finalizar muestra el promedio general del curso.
